import React, { useState } from "react";
import styled from "styled-components";
import AudioCard from "../card/Audiocard";

import foldIcon from "../../assets/img/icons/foldicon.svg";
import spreadIcon from "../../assets/img/icons/spreadicon.svg";

const followingUploadData = {
  page1: [
    {
      imageUrl:
        "data:image/jpeg;base64,",
      category: "NEW FOR YOU",
      title: "My New Arrivals",
      writer: "Deine Freunde, Moderat",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://upload.wikimedia.org/wikipedia/en/1/1b/Lauv_-_I_Like_Me_Better.png",
      category: "NEW TRACK FOR YOU",
      title: "Coexist",
      writer: "Album by the XX",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcTMATmyeAAbmMBWQl7k9Sy3q_xCz9-NimlzHygS5k4xov9yyiqA",
      category: "NEW ALBUM",
      title: "After Hours",
      writer: "The Weekend",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRjlUhv8Nek6yMkzivUQFT8pdJ0qjdzfD6oswjgnKgGn-L92cBK",
      category: "BASED ON YOUR LIKES",
      title: "If You Wait",
      writer: "London Grammar",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "data:image/jpeg;base64,",
      category: "NEW FOR YOU",
      title: "My New Arrivals",
      writer: "Deine Freunde, Moderat",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://upload.wikimedia.org/wikipedia/en/1/1b/Lauv_-_I_Like_Me_Better.png",
      category: "NEW TRACK FOR YOU",
      title: "Coexist",
      writer: "Album by the XX",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcTMATmyeAAbmMBWQl7k9Sy3q_xCz9-NimlzHygS5k4xov9yyiqA",
      category: "NEW ALBUM",
      title: "After Hours",
      writer: "The Weekend",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRjlUhv8Nek6yMkzivUQFT8pdJ0qjdzfD6oswjgnKgGn-L92cBK",
      category: "BASED ON YOUR LIKES",
      title: "If You Wait",
      writer: "London Grammar",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "data:image/jpeg;base64,",
      category: "NEW FOR YOU",
      title: "My New Arrivals",
      writer: "Deine Freunde, Moderat",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://upload.wikimedia.org/wikipedia/en/1/1b/Lauv_-_I_Like_Me_Better.png",
      category: "NEW TRACK FOR YOU",
      title: "Coexist",
      writer: "Album by the XX",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcTMATmyeAAbmMBWQl7k9Sy3q_xCz9-NimlzHygS5k4xov9yyiqA",
      category: "NEW ALBUM",
      title: "After Hours",
      writer: "The Weekend",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRjlUhv8Nek6yMkzivUQFT8pdJ0qjdzfD6oswjgnKgGn-L92cBK",
      category: "BASED ON YOUR LIKES",
      title: "If You Wait",
      writer: "London Grammar",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "data:image/jpeg;base64,",
      category: "NEW FOR YOU",
      title: "My New Arrivals",
      writer: "Deine Freunde, Moderat",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://upload.wikimedia.org/wikipedia/en/1/1b/Lauv_-_I_Like_Me_Better.png",
      category: "NEW TRACK FOR YOU",
      title: "Coexist",
      writer: "Album by the XX",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcTMATmyeAAbmMBWQl7k9Sy3q_xCz9-NimlzHygS5k4xov9yyiqA",
      category: "NEW ALBUM",
      title: "After Hours",
      writer: "The Weekend",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRjlUhv8Nek6yMkzivUQFT8pdJ0qjdzfD6oswjgnKgGn-L92cBK",
      category: "BASED ON YOUR LIKES",
      title: "If You Wait",
      writer: "London Grammar",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "data:image/jpeg;base64,",
      category: "NEW FOR YOU",
      title: "My New Arrivals",
      writer: "Deine Freunde, Moderat",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://upload.wikimedia.org/wikipedia/en/1/1b/Lauv_-_I_Like_Me_Better.png",
      category: "NEW TRACK FOR YOU",
      title: "Coexist",
      writer: "Album by the XX",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://encrypted-tbn3.gstatic.com/images?q=tbn:ANd9GcTMATmyeAAbmMBWQl7k9Sy3q_xCz9-NimlzHygS5k4xov9yyiqA",
      category: "NEW ALBUM",
      title: "After Hours",
      writer: "The Weekend",
      tag: "# Tag1",
    },
    {
      imageUrl:
        "https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcRjlUhv8Nek6yMkzivUQFT8pdJ0qjdzfD6oswjgnKgGn-L92cBK",
      category: "BASED ON YOUR LIKES",
      title: "If You Wait",
      writer: "London Grammar",
      tag: "# Tag1",
    },
  ],
};

const FollowingUpload = () => {
  const [isExpanded, setIsExpanded] = useState(false);

  const toggleExpand = () => {
    setIsExpanded(!isExpanded);
  };

  const followingData = followingUploadData.page1;

  const displayedData = isExpanded
    ? followingData.slice(0, 25)
    : followingData.slice(0, 10);

  return (
    <FollowingUploadWrapper>
      <FollowingTitle>팔로잉</FollowingTitle>
      <FollowingUploadHeader>
        <TextWrapper>
          <FollowingUploadTitle>팔로잉 업로드</FollowingUploadTitle>
          <FollowingUploadInfo>
            팔로잉 유저들의 다양한 게시물을 만나보세요
          </FollowingUploadInfo>
        </TextWrapper>
        <ExpandButton onClick={toggleExpand}>
          <img src={isExpanded ? foldIcon : spreadIcon} alt="toggle button" />
        </ExpandButton>
      </FollowingUploadHeader>
      <CardsContainer isExpanded={isExpanded}>
        {displayedData.map((data, index) => (
          <CardWrapper key={index} index={index} isExpanded={isExpanded}>
            <AudioCard {...data} />
          </CardWrapper>
        ))}
      </CardsContainer>
    </FollowingUploadWrapper>
  );
};

export default FollowingUpload;

const FollowingUploadWrapper = styled.div`
  padding: 220px 120px;
  margin: 0 auto;
  max-width: 1400px;
  box-sizing: border-box;
`;

const FollowingTitle = styled.span`
  font-size: 45px;
  font-weight: bold;
  text-align: left;
  color: #000000;
  width: 100%;
`;

const FollowingUploadHeader = styled.div`
  display: flex;
  flex-direction: row;
  justify-content: space-between;
  align-items: center;
  margin-top: 35px;
  margin-bottom: 20px;
`;

const TextWrapper = styled.div`
  display: flex;
  flex-direction: row;
  justify-content: start;
  align-items: center;
`;

const FollowingUploadTitle = styled.span`
  font-size: 30px;
  font-weight: bold;
  color: #000000;
`;

const FollowingUploadInfo = styled.span`
  font-size: 16px;
  margin-left: 20px;
  color: #777777;
`;

const ExpandButton = styled.button`
  background: none;
  border: none;
  cursor: pointer;
  display: flex;
  justify-content: center;
  align-items: center;
  padding: 10px;
`;

const CardsContainer = styled.div`
  display: grid;
  grid-template-columns: repeat(5, 1fr);
  grid-template-rows: ${({ isExpanded }) =>
    isExpanded ? "repeat(5, auto)" : "repeat(2, auto)"};
  grid-gap: 20px;
  width: 100%;
  margin: 0;
  padding: 0;
  transition: grid-template-rows 0.3s ease;

  @media (max-width: 1200px) {
    grid-template-columns: repeat(3, 1fr);
    grid-template-rows: ${({ isExpanded }) =>
      isExpanded ? "repeat(9, auto)" : "repeat(4, auto)"};
  }
  @media (max-width: 900px) {
    grid-template-columns: repeat(2, 1fr);
    grid-template-rows: ${({ isExpanded }) =>
      isExpanded ? "repeat(13, auto)" : "repeat(7, auto)"};
  }
  @media (max-width: 600px) {
    grid-template-columns: 1fr;
    grid-template-rows: ${({ isExpanded }) =>
      isExpanded ? "repeat(25, auto)" : "repeat(10, auto)"};
  }
`;

const CardWrapper = styled.div`
  transition: opacity 0.3s ease, transform 0.3s ease;
  opacity: 1;
  transform: translateY(0);
`;
